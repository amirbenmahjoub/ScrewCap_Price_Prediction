...
predict_price <- function(v, training_data_set) {
  
  res_famd = FAMD(training_data_set, ncp = 5, sup.var = c(10,7,1,5))
  new_data_set <- as.data.frame(res_famd$ind$coord)
  new_data_set$Price <- training_data_set$Price
  new_data_set <- new_data_set %>% rename(Dim1 = 'Dim.1', Dim2 = 'Dim.2',Dim3 = 'Dim.3',Dim4 = 'Dim.4', Dim5 = 'Dim.5')
  fit_final <- lm(Price ~ Dim1 + Dim2 + Dim3 + Dim4 + Dim5  , data= new_data_set)
  v_projected <- as.data.frame(predict.FAMD(res.famd, v)$coord)
  v_projected <- v_projected %>% rename(Dim1 = 'Dim 1', Dim2 = 'Dim 2',Dim3 = 'Dim 3',Dim4 = 'Dim 4', Dim5 = 'Dim 5')
  return(predict(fit_final, v_projected))
}



predict_price( dataset[143,] , dataset)
